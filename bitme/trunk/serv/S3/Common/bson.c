...
static void* 
__create_new_bson(lua_State *L,int size)
{
	...
	if (luaL_newmetatable(L, "bson")) {
		...
		lua_setfield(L, -2, "__unm");	// use the unary - operation to decode
...
// 注意：这个函数的执行结果是一个bson userdata数据，不是展开的lua表 
static int
ldecode2(lua_State *L) {
	size_t len = 0;
	...
	lua_pop(L,1);
	void* ud = __create_new_bson(L,len);
	if(0 == ud){
		lua_pushnil(L);
		return 0;
	}
...
ldecode(lua_State *L) {
	...
	int ud_len = lua_objlen(L,1);
	const int32_t * data = (const int32_t*)lua_touserdata(L,1);
	if (data == NULL) {
		return 0;
	}
	int32_t unsafe_len = *data;
	if(unsafe_len<1 || unsafe_len>ud_len){
		lua_pop(L,1);
		lua_pushnil(L);
		return 0;
	}
	
	struct bson_reader br = { (const uint8_t *)data , unsafe_len };
...
lencode(lua_State *L) {
	...
	pack_dict(L, &b, false);
	void* ud = __create_new_bson(L,b.size);
	if(ud){
	...
	if (luaL_newmetatable(L, "bson")) {
		...
		lua_pushcfunction(L, lmakeindex);
		lua_setfield(L, -2, "makeindex");
...
typeclosure(lua_State *L) {
	...
	int i;
	int n = sizeof(typenamea)/sizeof(typenamea[0]);
	for (i=0;i<n;i++) {
		lua_pushstring(L,typenamea[i]);
	}
