...
struct GameTime
{
    ...
    void            init(int active_mode = 1){
		...
		*/
		...
		struct timeval aa;
		gettimeofday(&aa,0);
		m_StartTime = ((u64)aa.tv_sec)*1000L+aa.tv_usec/1000;
		
		m_local_usec_start_time = ((u64)aa.tv_sec)*1000000L+aa.tv_usec;
		m_local_usec_time = 0;
    ...
    void            setTime(){
    	...
    	struct timeval aa;
	    gettimeofday(&aa,0);
	    u64 bb = ((u64)aa.tv_sec)*1000L+aa.tv_usec/1000;
	    m_LocalTime = timetype(bb - m_StartTime);
	    m_local_usec_time = ((u64)aa.tv_sec)*1000000L+aa.tv_usec - m_local_usec_start_time;
	    if(1==m_active_mode){
    ...
    //取得服务器端程序启动时的时间计数值
    u64        		getStartTime(){ return (timetype)m_StartTime ; }
